...
export const get = query({
  ...
  async handler(ctx, args) {
    ...
    const conversationMemberships = await ctx.db
      ...
      .withIndex("by_memberId", (e) => e.eq("memberId", currentUser._id))
      .collect();
    ...
    const conversationWithDetails = await Promise.all(
      (await conversations).map(async (conversation, index) => {
        const allConversationMemberships = await ctx.db
          .query("conversationMembers")
          .withIndex("by_conversationId", (e) =>
            e.eq("conversationId", conversation._id)
          )
          .collect();

        console.log(
          `ini conversation ${JSON.stringify(allConversationMemberships)}`
        );
